...
public:
  ...
  size_t Get(TAirport airport) const { return data[static_cast<size_t>(airport)]; }

  void Insert(TAirport airport){ ++GetRef(airport); }

  void EraseOne(TAirport airport){ --GetRef(airport); }

  void EraseAll(TAirport airport){ GetRef(airport) = 0; }
