...
public:
  ...
  Figure(const string& type) : Type(type) {}
...
{
public:
  Rect(int& w, int& h) : Figure("RECT") , P(2*w + 2*h) , S(w*h) {
  }

  string Name() override { return Type; }
  float Perimeter() override { return P; }
  long double Area() override { return S; }
